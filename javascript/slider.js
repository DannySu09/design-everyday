...
//    todo: 这和样式表强耦合，要找一个去得到样式表transition-duration属性值的办法
//    和css的相同，400ms
    option._duration = 400;
    return option;
};
...
Slider.prototype.disableTransition = function() {
    if(this.$inner.hasClass(u.classSet.transition) === true) {
        this.$inner.removeClass(u.classSet.transition);
    }
};

// 删除transition
Slider.prototype.enableTransition = function(){
    if(this.$inner.hasClass(u.classSet.transition) === false) {
        this.$inner.addClass(u.classSet.transition);
    }
};
...
Slider.prototype.triggerInfinite = function(){
    ...
    } else if(this._isInsertAfter && this._index >= this._length * (3/2)) {
...
Slider.prototype.go = function(index){
    ...
    var slideDuration = this.option._duration + 100;
// 如果不是数字，尝试转换为数字
    if(isNaN(index)) {
